...
$(document).ready(function() {
    ...
    function loadServersDataSmart() {
        if (isUpdating) {
            logging('⏳ 이미 업데이트 중입니다. 건너뜀.');
            return;
        }
        
        logging('🔄 스마트 서버 데이터 로딩 시작');
        isUpdating = true;
        ...
        $.getJSON(SERVERS_API)
            .then(function(response) {
                if (response.success) {
                    ...
                    logging(`📊 서버 필터링 결과: ${validServers.length}개 (전체: ${allServers.length}개)`);
                    
                    const hasChanges = updateServerStatusCache(validServers);
                    
                    if (hasChanges) {
                        logging('✅ 서버 상태 변경 감지됨. UI 업데이트');
                        ...
                        logging('ℹ️ 서버 상태 변경 없음. UI 업데이트 건너뜀');
                        // 변경이 없어도 시간은 업데이트
                        lastUpdateTime = new Date();
                        updateLastUpdateTime();
                    }
            ...
            .catch(function(error) {
                logging.error('❌ 서버 데이터 로딩 실패:', error);
                // 오류 발생 시 기본값 설정
                servers = [];
                loadServersOverview();
            })
    ...
    function updateServerStatusCache(newServers) {
        ...
        const validServers = newServers.filter(server => {
            // Cloud-init 서버 제외
            if (server.name && server.name.toLowerCase().includes('cloud-init')) {
                logging(`🚫 Cloud-init 서버 제외: ${server.name} (${server.ip})`);
                return false;
            }
            
            // IP가 0.0.0.0인 서버 제외
            if (server.ip === '0.0.0.0' || server.ip === '127.0.0.1') {
                logging(`🚫 로컬/무효 IP 서버 제외: ${server.name} (${server.ip})`);
                return false;
            }
            
            // 이름이 없는 서버 제외
            if (!server.name || server.name.trim() === '') {
                logging(`🚫 이름 없는 서버 제외: ${server.ip}`);
                return false;
            }
        ...
        logging(`📊 유효한 서버: ${validServers.length}개 (전체: ${newServers.length}개)`);
        ...
        validServers.forEach(server => {
            ...
            if (cachedStatus !== currentStatus) {
                logging(`🔄 서버 상태 변경: ${server.name} (${server.ip}) - ${cachedStatus} → ${currentStatus}`);
    ...
    function startBackgroundUpdate() {
        logging(`⏰ 백그라운드 업데이트 시작 (${updateInterval / 1000}초 간격)`);
    ...
    function handleVisibilityChange() {
        if (document.hidden) {
            logging('👁️ 페이지 숨김 - 백그라운드 업데이트 일시 중지');
            stopBackgroundUpdate();
        } else {
            logging('👁️ 페이지 표시 - 백그라운드 업데이트 재시작');
            startBackgroundUpdate();
    ...
    function loadServersData() {
        logging('⚠️ 레거시 loadServersData 호출됨. 스마트 버전으로 리다이렉트');
        loadServersDataSmart();
    }
    ...
    function updateSummaryPanels() {
        try {
            logging('updateSummaryPanels 호출됨');
            
            // 서버 데이터 검증
            if (!servers || !Array.isArray(servers)) {
                logging.warn('서버 데이터가 없거나 유효하지 않습니다.');
                servers = [];
            }
            ...
            logging(`서버 통계: 전체=${total}, 정상=${healthy}, 경고=${warning}, 위험=${critical}`);
            ...
            logging.error('요약 패널 업데이트 오류:', error);
            // 오류 발생 시 기본값 설정
    ...
    function populateServerDropdown() {
        try {
            logging('populateServerDropdown 호출됨');
            ...
            // 서버 데이터 검증
            if (!servers || !Array.isArray(servers)) {
                logging.warn('서버 드롭다운 초기화: 서버 데이터가 없습니다.');
                servers = [];
            }
            ...
            if (servers.length > 0) {
                ...
                logging(`${servers.length}개 서버 옵션 추가됨 (이름 기반)`);
            } else {
                // 서버가 없을 때 기본 옵션 추가
                select.append('<option value="none" disabled>서버가 없습니다</option>');
                logging('서버가 없어서 기본 옵션만 표시');
            }
            
        } catch (error) {
            logging.error('서버 드롭다운 초기화 오류:', error);
            // 오류 발생 시 기본 옵션만 표시
            const select = $('#server-select');
            select.empty();
            select.append('<option value="all">🖥️ 전체 서버</option>');
        ...
        $.getJSON('/monitoring/grafana-dashboard')
            .then(function(response) {
                if (response.success && response.data) {
                    ...
                    logging('Grafana 대시보드 정보 로드 완료:', grafanaDashboardInfo);
                    
                    // 초기 대시보드 표시
                    displayGrafanaDashboard('all');
                } else {
                    logging.error('Grafana 대시보드 정보 로드 실패:', response);
                    showGrafanaError('Grafana 대시보드 정보를 가져올 수 없습니다.');
                }
            })
            .catch(function(error) {
                logging.error('Grafana 대시보드 초기화 실패:', error);
    ...
    function displayGrafanaDashboard(selectedServer) {
        if (!grafanaDashboardInfo) {
            logging.warn('⚠️ Grafana 대시보드 정보가 없습니다.');
            return;
        }
        
        logging('📊 Grafana 대시보드 표시 시작:', {
        ...
        iframe.onerror = function() {
            logging.error('❌ Grafana 대시보드 로드 실패');
            $('#grafana-loading').hide();
    ...
    function generateGrafanaEmbedUrl(selectedServer) {
        ...
        if (selectedServer && selectedServer !== 'all') {
            logging(`🔍 서버 필터링 시도: ${selectedServer}`);
            ...
            logging(`✅ 서버 필터링 적용: ${selectedServer} -> ${serverFilters[0]}`);
            logging(`🔗 최종 URL: ${embedUrl}`);
        } else {
            logging(`📊 전체 서버 표시 모드`);
        }
    ...
    window.updateGrafanaDashboard = function() {
        ...
        if (selectedServer && grafanaDashboardInfo) {
            logging('🚀 Grafana 대시보드 업데이트 시작...');
            displayGrafanaDashboard(selectedServer);
        } else {
            logging.warn('⚠️ Grafana 대시보드 업데이트 실패:', {
                selectedServer: selectedServer,
    ...
    window.refreshGrafanaDashboard = function() {
        ...
        logging('Grafana 대시보드 새로고침:', selectedServer);
    ...
    window.testGrafanaVariables = function() {
        ...
        testFormats.forEach((format, index) => {
            ...
            logging(`📋 테스트 ${index + 1}: ${format}`);
            logging(`🔗 URL: ${testUrl}`);
        });
        
        logging('💡 위 URL들을 브라우저에서 직접 테스트해보세요!');
    };
    ...
    function displayServerAlerts() {
        try {
            ...
            logging(`✅ ${problematicServers.length}개 서버의 경고/위험 정보 표시 완료`);
            
        } catch (error) {
            logging.error('❌ 경고/위험 서버 상세 정보 표시 오류:', error);
